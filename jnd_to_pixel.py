...
tolerance = 5
pixel_corrected = []
for i in range(len(jnd_desired)):
    result = np.where(np.absolute(jnd_desired[i] - jnd_measured) < tolerance)
    pixel_corrected.append(np.round(np.mean(result)))

# Create training_data = (input, output) pairs
my_dataset = []
for i in range(len(jnd_measured)):
    my_dataset.append((jnd_measured[i], pixel_corrected[i]))
...
class PrintDot(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs):
        if epoch % 5000 == 0: print('')
        print('.', end='')


# Create checkpoint callback
checkpoint_path = r"C:\Users\Djalma\Google Drive\University\Doctorate - BME\Repository\display_calibration\jnd_to_pixel.ckpt"
...
# model = keras.models.load_model('jnd_to_pixel.h5')
